<template>
  <div class="flex items-center bg-white border border-gray-200 rounded-2xl px-3 py-2 w-full max-w-[300px] min-w-[250px] shadow-sm">
    <div class="flex-1 text-sm sm:text-base font-bold text-gray-800 tracking-wider relative overflow-hidden" style="font-family: 'Google Sans', sans-serif;">
      <div class="flex space-x-0.5 sm:space-x-1 relative">
        <div v-for="(group, groupIndex) in displayNumberGroups" :key="groupIndex" class="flex">
          <template v-for="(digit, digitIndex) in group.split('')" :key="`${groupIndex}-${digitIndex}`">
            <div class="number-slot relative overflow-hidden w-[10px] sm:w-[12px] h-[18px] sm:h-[20px]">
    ...
    <button 
      class="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 ml-7 sm:ml-8 rounded-xl transition-all duration-300 relative overflow-hidden"
      :class="{
...
export default {
  ...
  methods: {
    ...
    getTransitionDuration() {
      return 300; // 300ms for transition duration
    },
    getLastDigitRevealTime() {
      // Calculate total time to reveal all digits
      const lastIndex = 15; // 16 digits total (0-15)
      const baseDelay = 50;
    ...
    resetState() {
      ...
      // Reset isResetting after all animations are complete
      setTimeout(() => {
        this.isResetting = false;
      }, 1200); // Sufficient delay for all animations to complete
    },
    clearAllTimeouts() {
...
.global-shine-effect {
  ...
  /* Wait for all digits to be revealed (about 1150ms) before starting the effect */
